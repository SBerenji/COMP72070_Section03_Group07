...
Packet* ListingRoutes::handleUpdateListing(Listing listing) {
	...
	// Prepare SQL query to select user by username
	std::string query = "UPDATE listings SET title = '" + listing.getTitle() + "', description = '" + listing.getDescription() + "', price = '" + std::to_string(listing.getPrice()) + "', category = '" + listing.getCategory() + "', location = '" + listing.getLocation() + "', image = '" + base64Encode(listing.getImage()) + "' WHERE id = '" + std::to_string(listing.getId()) + "'";
